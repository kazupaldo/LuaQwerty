...
do
    ...
    if blocked then
        pcall(function()
            game:GetService("StarterGui"):SetCore("SendNotification", { Title = "Kazu Hub", Text = "Already running.", Duration = 4 })
        end)
        warn("[Kazu Hub] Already running.")
        return
    end
end
pcall(function()
...
do
    ...
    if not Library then
        error("[Kazu Hub] Obsidian failed to load after 4 tries. Re-execute. Last error: " .. tostring(lastErr))
    end
end
...
local window = Library:CreateWindow({
    Title = "Kazu Hub",
    Footer = "Anime Dice | Kazu Hub build 53",
...
local function log(...) print("[Kazu Hub]", ...) end
...
local CFG_FOLDER = "Kazu Hub"
local CFG_FOLDER_OLD = "AnimeDiceHub" -- legacy folder: read once, saves go to the new one
...
local function sendRollHook(url, e, cfg, chance)
    ...
    return postWebhook(url, { username = "Kazu Hub", embeds = { emb } })
end
...
local function sendStatsHook(url)
    ...
    return postWebhook(url, { username = "Kazu Hub", embeds = embeds })
end
...
do
    -- Obsidian uses groupboxes instead of direct tab controls.
    -- These small adapters preserve the original feature callbacks and saved-state code.
    local uiIndex = 0
    local function uiId(name)
        uiIndex += 1
        return "Kazu_" .. tostring(name):gsub("[^%w]", "") .. "_" .. uiIndex
    end

    local function wrapOption(raw, kind)
        local handle = { _raw = raw, Type = kind }
        return setmetatable(handle, {
            __index = function(_, key)
                if key == "value" then
                    if raw.Value ~= nil then return raw.Value end
                    return raw.Text
                elseif key == "Set" then
                    return function(_, value)
                        if kind == "label" then
                            raw:SetText(tostring(value))
                        else
                            raw:SetValue(value)
                        end
                    end
    ...
    local function makeConsole(group, info)
        local title = tostring(info.name or "Console")
        local lines = {}
        local initial = tostring(info.text or "")
        if initial ~= "" then table.insert(lines, initial) end
        local label = group:AddLabel({ Text = title .. "\n" .. initial, DoesWrap = true })
        local handle = { value = initial }
        function handle:Set(text)
            handle.value = tostring(text or "")
            label:SetText(title .. "\n" .. handle.value)
        end
        function handle:Append(text)
            table.insert(lines, tostring(text))
            local maxLines = tonumber(info.maxLines) or 120
            while #lines > maxLines do table.remove(lines, 1) end
            handle.value = table.concat(lines, "\n")
            label:SetText(title .. "\n" .. handle.value)
        end
        return handle
    end

    local function makeStat(group, info)
        local title = tostring(info.name or "Stat")
        local initial = tonumber(info.value) or 0
        local label = group:AddLabel({ Text = title .. ": " .. tostring(initial) })
        local handle = { value = initial, baseline = initial }
        function handle:Set(value)
            handle.value = tonumber(value) or 0
            label:SetText(title .. ": " .. tostring(handle.value))
        end
        function handle:ResetBaseline()
            handle.baseline = handle.value
        end
        return handle
    end
    ...
    local function addTab(name, icon)
        ...
        local proxy = {}
        function proxy:CreateDivider(info)
            group:AddLabel({ Text = tostring(info and info.text or ""), DoesWrap = false })
        end
        function proxy:CreateToggle(info)
            return wrapOption(group:AddToggle(uiId(info.name), {
                Text = info.name,
                Default = info.value,
        ...
        function proxy:CreateSlider(info)
            local range = info.range or { 0, 100 }
            return wrapOption(group:AddSlider(uiId(info.name), {
                Text = info.name,
                Min = range[1],
                Max = range[2],
                Default = info.value,
                Rounding = info.increment and ((info.increment < 1) and 1 or 0) or 0,
                Suffix = info.suffix,
                Callback = info.callback,
            }), "slider")
        end
        function proxy:CreateDropdown(info)
            return wrapOption(group:AddDropdown(uiId(info.name), {
                Text = info.name,
        ...
        function proxy:CreateInput(info)
            return wrapOption(group:AddInput(uiId(info.name), {
                Text = info.name,
        ...
        function proxy:CreateButton(info)
            return group:AddButton({
                Text = info.name,
                Func = info.callback,
            })
        end
        function proxy:CreateConsole(info)
            return makeConsole(group, info)
        end
        function proxy:CreateStat(info)
            return makeStat(group, info)
        end
        function proxy:CreateText(info)
            return group:AddLabel({
                Text = tostring(info.name or "") .. "\n" .. tostring(info.text or ""),
                DoesWrap = true,
            })
        end
        return proxy
    ...
    local tMain = addTab("Main", "home")
    local tUnits = addTab("Units", "users")
    local tUpg = addTab("Upgrades", "arrow-up-circle")
    local tDice = addTab("Dice", "dice-5")
    local tSell = addTab("Sell", "shopping-cart")
    local tTower = addTab("Tower", "castle")
    local tPotion = addTab("Potion", "flask-conical")
    local tReroll = addTab("Reroll", "rotate-ccw")
    local tTrade = addTab("Trade", "handshake")
    local tStats = addTab("Stats", "bar-chart-3")
    local tChanges = addTab("Changelogs", "history")
    local tSettings = addTab("Settings", "settings")
    local tHooks = addTab("Webhook", "webhook")
...
<font color="#4ade80">•</font> Kazu Hub per-player settings, restored automatically on the next run
...
U.fpsOn = tSettings:CreateToggle({ name = "FPS Boost", description = "Hides leaderboards, Best Roll podium + side statues, Towers building, other players/bases, outer islands, all trees/rocks/clovers/decor (lobby, stalls, yours + Shop stay). Plus shadows/particles/mesh-textures/lights/postfx off, render quality lowered. FPS cap is never touched. Saved and re-applied on rejoin.",
    ...
    tSettings:CreateDivider({ text = "Kazu Hub Settings" })
    tSettings:CreateText({
        name = "Per-player settings",
        text = "Settings are saved to the Kazu Hub folder for this account and restored automatically on the next run.",
    })
    tSettings:CreateButton({ name = "Save Settings Now", callback = function()
        notify("Kazu Hub", saveNow() and "Settings saved." or "Nothing changed or file APIs are unavailable.")
    end })
...
notify("Kazu Hub", "Loaded (build " .. tostring(BUILD) .. "). Pick a tab and enable features.")
